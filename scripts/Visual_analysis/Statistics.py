...
def DataFrame_reformat(result_DF: pd.DataFrame) -> pd.DataFrame:

    result_dict_reformat = {'R2': [], 'MSE': [], 'eval_length': []}  # 初始化结果字典

    eval_length = [500, 1000, 1500, 2000, 2500, 3000]  # 定义评估长度
...
if __name__ == '__main__':
    ...
    result_file = f'{result_dir}/Analog-ESN_statistic_result/ESN_statistic_results_500.csv'  # 结果文件路径
    result_DF = pd.read_csv(result_file)  # 读取结果文件

    result_DF = DataFrame_reformat(result_DF)  # 重构DataFrame

    grid = sns.FacetGrid(result_DF, col='eval_length', col_wrap=3, height=2, sharex=False)  # 创建一个网格图 (x轴不共享)
    grid.map(sns.histplot, 'MSE', kde=True)  # 绘制每个子图的直方图

    for fmt in ['eps', 'png', 'pdf']:
        plt.savefig(f'{result_dir}/Analog-ESN_statistic_result/ESN_statistic_results.{fmt}')
