...
class ERA:
    ...
    def TrainEncoder(self, bias: bool = False, optimizer_mode: str = 'Adam', lr: float = 0.001,
                     loss_function: nn.Module = an.API_for_PyTorch.Loss_RMSE(), num_epochs: int = 2000):
        '''
        Echo State Reservoir Encoder训练模块 (回声状态储层编码器训练模块)，此函数可以根据给定的训练集轨迹产生对应的储层轨迹
        '''
        # V_train, Y_train = (self.V_train, self.Y_train)  # 训练阶段的储层输出，训练集输出
        ...
        # 设置优化器
        optimizer = an.API_for_PyTorch.PyTorch_optimizer(output_projection.parameters(), optimizer=optimizer_mode,
                                                         learning_rate=lr)
        # loss_function = loss_function  # 定义loss函数

        # 开始训练模型
        self.loss_record_encoder = np.zeros((num_epochs, 2), dtype=np.float32)  # 创建一个零数组用于记录encoder训练的loss
        # 将numpy数组转换为tensor
        input = torch.from_numpy(self.V_train)
        target = torch.from_numpy(self.Y_train)
        for epoch in tqdm(range(num_epochs)):
            optimizer.zero_grad()  # 梯度要清零每一次迭代
            output: torch.Tensor = output_projection(input)  # 前向传播
        ...
        self.ERA_train = output_projection(torch.from_numpy(self.V_train)).detach().numpy()  # 训练集的ESN输出

        return self.ERA_train.T, output_projection  # 对于网络输出，需要转置以适配ASHEN的数据结构

    # 迭代验证 ----------------------------------------------------------------------------------------------------------

    def EchoStateIterator(self, output_projection: nn.Module):
        '''
        ...
        # ERA_valid = torch.zeros((self.L_valid, N), dtype=torch.float32)  # Y_train为预测阶段的输出
    ...
    def TrainIterator(self, output_projection: nn.Module, optimizer_mode: str = 'Adam', lr: float = 0.001,
                      loss_function: nn.Module = an.API_for_PyTorch.Loss_RMSE(), num_epochs: int = 100):
        '''
        Echo State Reservoir Iterator训练模块 (回声状态储层迭代器训练模块)，此函数可以通过给定的输出连接output_projection对储层进行迭代
        '''
        optimizer = an.API_for_PyTorch.PyTorch_optimizer(output_projection.parameters(), optimizer=optimizer_mode,
                                                         learning_rate=lr)
        # loss_function = an.API_for_PyTorch.Loss_MSE()  # 定义loss函数

        # 开始训练模型
        self.loss_record_iterator = np.zeros((num_epochs, 2), dtype=np.float32)  # 创建一个零数组用于记录loss
        # 将numpy数组转换为tensor
        # input = torch.from_numpy(V_valid)        # 预训练完的验证集的储层输出（The pre-trained echo state reservoir output）
        target = torch.from_numpy(self.Y_valid)  # 验证集的输出（The ground truth output of the validation set）
        for epoch in tqdm(range(num_epochs)):
            optimizer.zero_grad()  # 梯度要清零每一次迭代
            # 通过迭代器产生验证集的储层态输出
            ERA_valid, V_valid, R_valid, r_state_iterated = self.EchoStateIterator(output_projection)
            # 前向传播，此时的ESN_valid为numpy数组，需将其转换为pytorch的tensor，而且要加入gradient才能利用pytorch的自动微分功能进行反向传播优化
            # output: torch.Tensor = torch.tensor(list(ESN_valid), requires_grad=True, dtype=torch.float32)  # 前向传播
            output: torch.Tensor = torch.tensor(ERA_valid, requires_grad=True, dtype=torch.float32)  # 前向传播
            # output: torch.Tensor = ESN_valid  # 前向传播

            loss = loss_function(target, output)  # 计算loss
            loss.backward()  # 返向传播
            optimizer.step()  # 更新权重参数

            self.loss_record_iterator[epoch] = (epoch, loss.item())  # 记录此epoch的loss
            epoch += 1  # 计数器加一

        # 利用训练好的网络进行预测，同时将所有输出都转化为实例变量，方便后续分析时按需输出
        self.ERA_valid, self.V_valid, self.R_valid, self.r_state_iterated = self.EchoStateIterator(output_projection)

        return self.ERA_valid.T, output_projection  # 对于网络输出，需要转置以适配ASHEN的数据结构

    # 外推预测 ----------------------------------------------------------------------------------------------------------

    def EchoStatePredictor(self, trained_output_projection: nn.Module, L_test: int):
        '''
        ...
        for i in range(L_test):
            ...
            v_1 = torch.from_numpy(v_1).float()  # 转换为pytorch的tensor，同时通过.float()确保其为32位浮点数张量，防止计算出错
            y = trained_output_projection(v_1)
            y = y.detach().numpy()  # 转换为numpy的array
    ...
    def GetLossRecord(self):
        ...
        获取训练过程的loss记录
        '''
        return np.vstack((self.loss_record_encoder, self.loss_record_iterator))  # 将训练和迭代的loss记录合并
