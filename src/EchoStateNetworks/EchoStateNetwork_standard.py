...
class ESN():
    ...
    '''
    def __init__(self, X_train: np.ndarray, Y_train: np.ndarray, activation: callable=np.tanh, input_scaling: float=0.1,
                 bias: float=0, network_mode: str='ER', res_dim: int=400, spec_rad: float=1.0, res_den: float=0.10,
                 transient: int=1000, fixed_random_number: bool=True, random_seed: int=42, memory_capacity_config=None,
                 **kwargs):
    ...
    # ------------------------------------------------------------------------------------------------------------------
    def Training(self, opt_algorithm: int=0):
        '''
        This function is designed for training the echo state network model.
        '''
        self.R_train = np.zeros((self.K, self.L_train), dtype=float)  # 初始化一个矩阵用于存放储层态

        # 更新初始储层态
        self.R_train[:,0] = self.func(np.dot(self.rho * self.W_res, self.R_train[:,0])
                                      + self.s_in * np.dot(self.W_in, self.X_train[:,0]) + self.b)
        for i in range(1, self.L_train):
            self.R_train[:,i] = self.func(np.dot(self.rho * self.W_res, self.R_train[:,i-1])
                                          + self.s_in * np.dot(self.W_in, self.X_train[:,i]) + self.b)

        # print(self.Y_train[:, self.transient:].T.shape,R_state[:, self.transient:].T.shape)  # For debugging
        W_out = WeightOptimizing(self.Y_train[:, self.transient:].T, self.R_train[:, self.transient:].T,
        ...
        self.init_input_test = self.Y_train[:, -1]  # 训练集的最后一个输出用于预测的初始输入
